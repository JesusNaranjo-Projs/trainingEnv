...
def compute_team_rewards_static(obs, prev_ball):
    """
    Compute reward_A and reward_B based on supplied observations.
    This will operate under the assumption that there is no environment to query
    ...
    def approach_reward(robot_pos, angle_to_ball, ball_pos):
        if angle_to_ball < math.pi / 4:
            return 0.2 / (dist(robot_pos, ball_pos) + 1e-5)
        return 100
    ...
    # 0-35 are lidar, 36-38 are x,y,angle, 39-40 are ball x,y, 41-42 are dist to goals
    ball_pos = obs[39:41]
    goalA_pos = [0.0, -0.585]
    goalB_pos = [0.0, 0.585]
    robotA_pos = obs[36:38]
    angle_to_ball = obs[38]
    ...
    if prev_ball is not None:
        reward_A += ball_movement_reward(prev_ball, ball_pos, goalB_pos)
        # reward_B += ball_movement_reward(env.prev_ball_pos, ball_pos, goalA_pos)
    ...
    # # [6] Approach reward (encouraging getting close *while* facing)
    reward_A += approach_reward(robotA_pos, angle_to_ball, ball_pos)
    # reward_B += approach_reward(robotB_pos, yawB, ball_pos)
